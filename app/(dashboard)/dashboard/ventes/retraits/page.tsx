...
import { Loader2, Search, ShoppingBag, AlertTriangle, CheckCircle, Package, Clock, ArrowLeft, X } from 'lucide-react'
...
type RetraitLigne = { produitId: number; designation: string; quantite: number; max: number; prixUnitaire: number }
...
export default function RetraitsPage() {
  ...
  const [retraitLignes, setRetraitLignes] = useState<RetraitLigne[]>([])
  const [retraitDate, setRetraitDate] = useState(new Date().toISOString().slice(0, 10))
  ...
  const fetchVentes = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/ventes?retraits=true')
  ...
  const stats = ventes.reduce((acc, v) => {
    ...
    acc.total++
    if (tl >= tq) acc.completed++
    else if (tl > 0) acc.partial++
    else acc.pending++
    return acc
  ...
  const filtered = ventes.filter(v => {
    const tq = v.lignes.reduce((s, l) => s + l.quantite, 0)
    const tl = v.lignes.reduce((s, l) => s + (l.quantiteLivree || 0), 0)
    if (viewMode === 'pending' && (tl >= tq || tl > 0)) return false
    if (viewMode === 'partial' && (tl <= 0 || tl >= tq)) return false
    if (viewMode === 'done' && tl < tq) return false
  ...
  function openModal(v: Vente) {
    ...
    setRetraitDate(new Date().toISOString().slice(0, 10))
    setModalOpen(true)
  }

  function updateQte(produitId: number, val: number) {
    setRetraitLignes(prev => prev.map(l => l.produitId === produitId ? { ...l, quantite: Math.max(0, Math.min(val, l.max)) } : l))
  }

  async function handleSubmitRetrait() {
    if (!selectedVente) return
    const lignes = retraitLignes.filter(l => l.quantite > 0)
    ...
    try {
      const res = await fetch(`/api/ventes/${selectedVente.id}`, {
        ...
        body: JSON.stringify({ action: 'RETRAIT_PARTIEL', lignes: lignes.map(l => ({ produitId: l.produitId, quantite: l.quantite })), date: retraitDate || undefined }),
      })
      if (!res.ok) {
      ...
      setSuccessMsg(`Retrait effectué sur ${selectedVente.numero}`)
      setModalOpen(false)
      fetchVentes()
                      ...
                      <tr key={v.id} className="hover:bg-white/5 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center">
                          {reste > 0 ? (
                            ...
                            <span className="text-xs text-green-300 font-medium">Complété</span>
                          )}
                        </td>
                      </tr>
                    )
      ...
      {modalOpen && selectedVente && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="sticky top-0 bg-orange-600 text-white px-6 py-4 rounded-t-2xl flex items-center justify-between">
              <h2 className="text-lg font-bold flex items-center gap-2">
                <ShoppingBag className="h-5 w-5" /> Retrait partiel — {selectedVente.numero}
              </h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Date du retrait</label>
                <input type="date" value={retraitDate} onChange={e => setRetraitDate(e.target.value)}
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none" />
              </div>
              <div className="space-y-3">
                <p className="text-sm font-medium text-gray-700">Produits à retirer</p>
                {retraitLignes.map(l => (
                  <div key={l.produitId} className="flex items-center gap-3 rounded-lg bg-gray-50 p-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500">Max: {l.max} / Prix: {l.prixUnitaire.toLocaleString('fr-FR')} F</p>
                    </div>
                    <input type="number" min={0} max={l.max} value={l.quantite}
                      onChange={e => updateQte(l.produitId, Number(e.target.value))}
                      className="w-20 rounded-lg border border-gray-300 px-2 py-1.5 text-sm text-right focus:border-orange-500 focus:outline-none" />
                  </div>
                ))}
              </div>
              {retraitLignes.filter(l => l.quantite > 0).length > 0 && (
                <div className="rounded-lg bg-orange-50 border border-orange-200 p-3 text-sm text-orange-800">
                  <strong>Total à retirer :</strong> {retraitLignes.filter(l => l.quantite > 0).length} produit(s) —
                  {retraitLignes.filter(l => l.quantite > 0).reduce((s, l) => s + l.quantite * l.prixUnitaire, 0).toLocaleString('fr-FR')} F
                </div>
              )}
              <button onClick={handleSubmitRetrait} disabled={submitting || retraitLignes.filter(l => l.quantite > 0).length === 0}
                className="w-full rounded-xl bg-orange-600 hover:bg-orange-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold py-3 px-4 transition-colors flex items-center justify-center gap-2">
