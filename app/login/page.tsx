      ...
      <div className="relative z-10 w-full max-w-5xl rounded-3xl shadow-2xl flex items-start">
        ...
        <div className="w-[420px] min-w-[360px] bg-white rounded-r-3xl p-8 md:p-10">
          <div className="text-center mb-6">
            <div className="flex items-center justify-center mb-3">
              <div className="relative">
                ...
                <div className="relative bg-gradient-to-br from-orange-500 to-orange-700 rounded-full p-5 shadow-lg">
                  <Image
                    src="/logo.png"
            ...
            <p className="text-orange-700 font-semibold text-base">Connexion sécurisée</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    ...
                    <p className="mt-1">
                      ...
                      {timeRemaining > 0 && <span className="font-bold">{formatTime(timeRemaining)}</span>}
                    </p>
                  </div>
                  <button type="button" onClick={resetLock} className="text-xs text-orange-600 hover:text-orange-700 underline font-medium whitespace-nowrap">Débloquer</button>
                </div>
              </div>
            )}

            <div>
              <label htmlFor="login" className="block text-sm font-medium text-gray-700 mb-1.5">Identifiant</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><User className="h-5 w-5 text-gray-400" /></div>
                <input id="login" type="text" value={login} onChange={(e) => setLogin(e.target.value)} required disabled={isLocked} autoComplete="username"
                  className="w-full rounded-xl border-2 border-gray-300 pl-10 pr-4 py-2.5 focus:border-orange-500 focus:ring-4 focus:ring-orange-500/20 outline-none transition-all duration-300 disabled:bg-gray-100 disabled:cursor-not-allowed hover:border-orange-400 text-sm"
                  placeholder="Votre identifiant" />
              </div>
            </div>

            <div>
              <label htmlFor="motDePasse" className="block text-sm font-medium text-gray-700 mb-1.5">Mot de passe</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Lock className="h-5 w-5 text-gray-400" /></div>
                <input id="motDePasse" type={showPassword ? 'text' : 'password'} value={motDePasse} onChange={(e) => setMotDePasse(e.target.value)} required disabled={isLocked} autoComplete="current-password"
                  className="w-full rounded-xl border-2 border-gray-300 pl-10 pr-12 py-2.5 focus:border-orange-500 focus:ring-4 focus:ring-orange-500/20 outline-none transition-all duration-300 disabled:bg-gray-100 disabled:cursor-not-allowed hover:border-orange-400 text-sm"
                  placeholder="••••••••" />
            ...
            {attempts > 0 && attempts < 5 && (
              <div className="text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2">
                ⚠️ Tentative {attempts} sur 5.
              </div>
            )}

            <button type="submit" disabled={loading || isLocked}
              className="w-full rounded-xl bg-gradient-to-r from-orange-600 to-orange-700 hover:from-orange-700 hover:to-orange-800 text-white font-semibold py-3 transition-all duration-300 shadow-lg hover:shadow-2xl hover:shadow-orange-500/50 hover:scale-[1.02] disabled:opacity-60 disabled:cursor-not-allowed disabled:shadow-none disabled:scale-100 transform text-sm">
              {loading ? (
          ...
          <div className="mt-3 text-center">
            <p className="text-xs text-gray-400">🔒 Connexion sécurisée par JWT</p>
          </div>
        </div>
      </div>
    </div>
