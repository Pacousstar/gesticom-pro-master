...
interface AnnulerVenteModalProps {
  isOpen: boolean
  onClose: () => void
...
export default function AnnulerVenteModal({
  isOpen,
  onClose,
  onConfirm,
  numero,
      ...
      <div className="mx-4 w-full max-w-lg rounded-2xl border border-red-200 bg-white shadow-2xl">
        <div className="flex items-start justify-between border-b border-gray-100 p-5">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-gray-900">Annuler la vente {numero} ?</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-4 p-5">
          <p className="text-sm text-gray-600">
            La vente <strong>{numero}</strong>{montantTotal > 0 ? <> de <strong>{montantTotal.toLocaleString('fr-FR')} F</strong></> : ''} sera annulée
            {montantPaye > 0 ? <> (dont {montantPaye.toLocaleString('fr-FR')} F déjà réglés)</> : ''}. Les stocks seront recrédités et les écritures comptables supprimées.
          </p>
          ...
          <p className="text-center text-xs font-bold text-red-600">
            La vente sera marquée « Annulée » mais conservée dans l'historique.
          </p>

          <div>
            <label className="mb-1 block text-xs font-medium text-gray-500">
              Tapez <span className="font-bold text-red-600">ANNULER</span> pour confirmer
            </label>
            <input
              type="text"
              value={typedText}
        ...
        <div className="flex items-center justify-end gap-3 border-t border-gray-100 p-5">
          ...
          >
            ...
            {loading ? 'Annulation en cours...' : "Annuler la vente"}
          </button>
        </div>
      </div>
    </div>
