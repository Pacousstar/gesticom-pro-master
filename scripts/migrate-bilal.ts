...
async function main() {
  console.log('=== Migration base Bilal (sans repair stock) ===\n')
  ...
  for (const s of fixes) {
    try { await p.$executeRawUnsafe(s) } catch {}
  }
  console.log('Data fixes appliquées')

  // prisma db push final
  execSync('npx prisma db push --accept-data-loss --skip-generate', {
    cwd: process.cwd(), stdio: 'pipe', timeout: 60000,
  })
  console.log('prisma db push réussi')
